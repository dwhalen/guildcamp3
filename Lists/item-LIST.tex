...
\NEW{Item}{\iInvisibilityCloak}{
	...
	\s\MYnumber	{10057}
	\s\MYtext	{Requires {$\omega$} = 2. This cloak enables the wearer to become invisible for five minutes or until they interact with a person or item. This means you can move around gamespace and here things, but no one sees you. Consume when used.}
}

\NEW{Item}{\iCloakingDevice}{
	\s\MYname	{Cloaking Device.}
	\s\MYnumber	{10058}
	\s\MYtext	{Requires {$\alpha$} = 2. Using this device renders the user invisible for five minutes or until they interact with a person or item. This means you can move around gamespace and here things, but no one sees you. Consume when used.}
}
